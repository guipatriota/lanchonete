...
def gerenciar_clientes():
    '''Mostra título do sub-menu Clientes'''
    print("Gerenciando clientes...")
    return 0

def gerenciar_pedidos():
    '''Mostra título do sub-menu Pedidos'''
    print("Gerenciando pedidos...")
    return 0

def gerenciar_estoque():
    '''Mostra título do sub-menu Estoque'''
...
def adicionar_pedido():
    ...
    print(f"\nO pedido com status {status}, hora {data_hora} do cliente",
          f"{id_cliente} será salvo no sistema.")
    conf = None
    while (conf not in ["y","n"]):
        conf = input("Deseja continuar ([Y]/n)? ").lower().strip()
        if conf in ("y", ""):
            while True:
                option = input("Se você deseja buscar o ID e nome dos "+
                         "itens no cardápio, digite 1 (0 para cancelar): ")
                if option == "1":
                    listar_cardapio()
                elif option == "0":
                    conf = "n"
                    break
                nome_cardapio= input(
                                "Digite o Nome do item do cardápio que você "+
                                "deseja adicionar ao pedido: ")
                id_cardapio = cardapio.get_id_por_nome(nome_cardapio)
                if id_cardapio:
                    id_cardapio = id_cardapio[0]
                    try:
                        quantidade = int(input(
                            "Digite a quantidade para o item "+
                            f"{nome_cardapio}: "))
                    except:
                        quantidade =0
                    ...
                    print(type(id_cardapio[0]))
                    print(type(quantidade))
                    pedido_cardapio.adicionar(id_pedido, id_cardapio[0], quantidade)
                    print(f"Produto {id_cardapio} com quantidade {quantidade}",
                        f"adicionado ao pedido {id_pedido}.")
...
def listar_pedido_por_status():
    ...
    for item in pedidos:
        print(f"ID: {item[0]}, Data e Hora:",
              f"{item[2]}, ID do Cliente: {item[3]}")
    input("\nPressione enter para continuar")
    return 1

def atualizar_pedido():
    '''Atualizar status de um pedido'''
    if input("Se você deseja buscar o ID do pedido antes de "+
              "atualizar os status, digite 1: ") == "1":
    ...
    print(f"\nO status do pedido {id_pedido}",
          f"será atualizado para {novo_status}.")

    conf = None
    while (conf not in ["y", "n"]):
        conf = input("Deseja continuar ([Y]/n)? ").lower().strip()
        if conf in ("y", ""):
            pedido.atualizar_status(id_pedido, novo_status)
...
def adicionar_produto_insumo(id_produto):
    ...
    while True:
        ...
        if id_insumo:
            quantidade_insumo = float(input(
                (f"Digite a quantidade do insumo {nome_insumo}",
                 "necessário para fabricação de 1 unidade do produto: ")))
        ...
        if insumos_adicionados == 0:
            ...
            adiciona_mais = input(
                "Deseja adicionar mais insumos? ([Y]/n): "
                ).lower().strip()
            if adiciona_mais == 'n':
                break
...
def adicionar_produto():
    ...
    print(f"\nO produto {nome} com quantidade {quantidade}, valor {valor}",
          f" e unidade de medida {unidade_medida} será salvo no sistema.")
    conf = None
    while (conf not in ["y","n"]):
        conf = input("Deseja continuar ([Y]/n)? ").lower().strip()
        if conf == "y" or conf == "":
...
def listar_produtos():
    ...
    print("-" * 40)
    print(f"{'ID':<5}{'Nome':<15}{'Quantidade':<10}",
          f"{'Valor':<10}{'Unidade de Medida':<10}")
    print("-" * 40)

    for prod in produtos:
        print(f"{prod[0]:<5}{prod[1]:<15}",
              f"{prod[2]:<10}{prod[3]:<10}{prod[4]:<10}")
...
def adicionar_item_cardapio():
    ...
    disponibilidade = int(input(
        "O item está disponível? (1 para sim, 0 para não): "))
    conf = None
    while conf not in ("y", "n"):
        conf = input("Deseja continuar ([Y]/n)? ").lower().strip()
        if conf in ("y", ""):
...
def adicionar_item_insumos():
    ...
    print(f"\nO insumo {nome} com {quantidade} {unidade_medida}",
          f"ao preço de {preco} será salvo no sistema.")
...
def listar_insumos():
    ...
    # Cabeçalho
    print(f'{"ID":<5} {"Nome":<20} {"Categoria":<20}',
          f'{"Unidade":<10} {"Quantidade":<12} {"Preço":<10}')
    ...
    for row in lista_insumos:
        ...
        print(f'{id_insumo:<5} {nome:<20} {categoria:<20}',
              f'{unidade_medida:<10} {quantidade:<12} {preco:<10}')
    input("\nPressione enter para continuar")
    return 1
